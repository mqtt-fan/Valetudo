...
HA_STATE_MAPPINGS[attributes.StatusAttribute.VALUE.ERROR] = HA_STATES.ERROR;
HA_STATE_MAPPINGS[attributes.StatusAttribute.VALUE.DOCKED] = HA_STATES.DOCKED;
HA_STATE_MAPPINGS[attributes.StatusAttribute.VALUE.IDLE] = HA_STATES.IDLE;
HA_STATE_MAPPINGS[attributes.StatusAttribute.VALUE.RETURNING] = HA_STATES.RETURNING;
HA_STATE_MAPPINGS[attributes.StatusAttribute.VALUE.CLEANING] = HA_STATES.CLEANING;
HA_STATE_MAPPINGS[attributes.StatusAttribute.VALUE.PAUSED] = HA_STATES.PAUSED;
HA_STATE_MAPPINGS[attributes.StatusAttribute.VALUE.MANUAL_CONTROL] = HA_STATES.CLEANING;
HA_STATE_MAPPINGS[attributes.StatusAttribute.VALUE.MOVING] = HA_STATES.CLEANING;
...
class MqttClient {
    ...
    updateAttributesTopicOnEvent(robotState) {
        const statusAttribute = robotState.getFirstMatchingAttributeByConstructor(attributes.StatusAttribute);
        ...
        if(statusAttribute) {
            this.last_ha_state = HA_STATE_MAPPINGS[statusAttribute.value];
            this.last_state = statusAttribute.value;

            if(statusAttribute.flag !== attributes.StatusAttribute.FLAG.NONE) {
                this.last_state += " " + statusAttribute.flag;
            }
        }

        if(areaStatistic) {
            this.last_cleaned_area = parseFloat((areaStatistic.value / 10000).toFixed(2))
        } else {
            this.last_cleaned_area = 0;
        }

        if(durationStatistic) {
            this.last_cleaning_time = parseFloat((durationStatistic.clean_time / 60).toFixed(1))
        } else {
            this.last_cleaning_time = 0;
        }
    ...
    updateStatusTopic(robotState) {
        const statusAttribute = robotState.getFirstMatchingAttributeByConstructor(attributes.StatusAttribute);
        const batteryAttribute = robotState.getFirstMatchingAttributeByConstructor(attributes.BatteryAttribute);
        const fanSpeedAttribute = robotState.getFirstMatchingAttributeByConstructor(attributes.FanSpeedAttribute);
        const areaStatistic = robotState.getFirstMatchingAttribute({
            attributeClass: attributes.LatestCleanupStatisticsAttribute.name,
            attributeType: attributes.LatestCleanupStatisticsAttribute.TYPE.AREA
        });
        ...
        if (this.client && this.client.connected === true) {
            ...
            if(statusAttribute) {
                response.state = HA_STATE_MAPPINGS[statusAttribute.value];

                if(statusAttribute.value === attributes.StatusAttribute.VALUE.ERROR) {
                    response.error = statusAttribute.metaData.error_description;
                }

                if(statusAttribute.flag !== attributes.StatusAttribute.FLAG.NONE) {
                    response.state += " " + statusAttribute.flag;
                }
            }

            if(batteryAttribute) {
                response.battery_level = batteryAttribute.level;
            }

            if(fanSpeedAttribute) {
                response.fan_speed = fanSpeedAttribute.value;
            }

            if(areaStatistic) {
                response.cleaned_area = (areaStatistic.value / 10000).toFixed(1);
            } else {
                response.cleaned_area = 0;
            }

            if(durationStatistic) {
                response.cleaning_time = (durationStatistic.value / 60).toFixed(1);
            } else {
        ...
        try {
            switch (command) {
                case MQTT_COMMANDS.START:
                    ...
                    /*
                        TODO: This roborock-specific logic has to be moved somewhere
