...
class Roborock extends MiioVacuum {
        ...
        if (now.getTime() - 600 > this.lastMapPoll.getTime()) {
            ...
            this.sendCloud({"method": "get_map_v1"}).then(res => {
                if (Array.isArray(res) && res.length === 1) {
                    ...
                    let statusAttribute = this.robotState.getFirstMatchingAttribute({
                        attributeClass: entities.state.attributes.StatusAttribute.name
                    });

                    if(statusAttribute && statusAttribute.isActiveState) {
                        repollSeconds = 2;
                    }
    ...
    parseStatus(res) {
        ...
        if (!matchingFanSpeed) {
            matchingFanSpeed = entities.state.attributes.FanSpeedAttribute.VALUE.CUSTOM;
        }

        let persistentMapSetting = entities.state.attributes.PersistentMapSettingAttribute.VALUE.DISABLED;
        if(res["lab_status"] === 1) {
            persistentMapSetting = entities.state.attributes.PersistentMapSettingAttribute.VALUE.ENABLED;
        }

        if(statusValue === entities.state.attributes.StatusAttribute.VALUE.PAUSED && res["in_cleaning"] !== 0) {
            statusFlag = entities.state.attributes.StatusAttribute.FLAG.RESUMABLE;

            if(res["in_cleaning"] === 2) {
                statusMetaData.zoned = true; //TODO!!
            }
        } else if(statusValue === entities.state.attributes.StatusAttribute.VALUE.ERROR) {
            statusMetaData.error_code = res["error_code"];
            statusMetaData.error_description = GET_ERROR_CODE_DESCRIPTION(res["error_code"]);
        }

        this.robotState.upsertFirstMatchingAttribute(new entities.state.attributes.StatusAttribute({
            value: statusValue,
            flag: statusFlag,
            metaData: statusMetaData
        }));

        this.robotState.upsertFirstMatchingAttribute(new entities.state.attributes.BatteryAttribute({
            level: res["battery"] || 0
        }));

        this.robotState.upsertFirstMatchingAttribute(new entities.state.attributes.FanSpeedAttribute({
            value: matchingFanSpeed,
            customValue: matchingFanSpeed === entities.state.attributes.FanSpeedAttribute.VALUE.CUSTOM ? res["fan_power"] : undefined
        }));
        ...
        this.robotState.upsertFirstMatchingAttribute(new entities.state.attributes.PersistentMapSettingAttribute({
    ...
    async startCleaningZoneByCoords(zones) {
        if (Array.isArray(zones) && zones.length <= 5) {
            const flippedZones = zones.map(zone => {
                const yFlippedZone = [
                    zone[0],
                    RRMapParser.DIMENSION_MM - zone[1],
                    zone[2],
                    RRMapParser.DIMENSION_MM - zone[3],
                    zone[4]
                ];
...
const STATUS_MAP = {
    1: {
        value: entities.state.attributes.StatusAttribute.VALUE.IDLE
    },
    2: {
        value: entities.state.attributes.StatusAttribute.VALUE.IDLE
    },
    ...
    4: {
        value: entities.state.attributes.StatusAttribute.VALUE.MANUAL_CONTROL
    },
    5: {
        value: entities.state.attributes.StatusAttribute.VALUE.CLEANING
    },
    6: {
        value: entities.state.attributes.StatusAttribute.VALUE.RETURNING
    },
    7: {
        value: entities.state.attributes.StatusAttribute.VALUE.MANUAL_CONTROL
    },
    8: {
        value: entities.state.attributes.StatusAttribute.VALUE.DOCKED
    },
    9: {
        value: entities.state.attributes.StatusAttribute.VALUE.IDLE
    },
    10: {
        value: entities.state.attributes.StatusAttribute.VALUE.PAUSED
    },
    11: {
        value: entities.state.attributes.StatusAttribute.VALUE.CLEANING,
        flag: entities.state.attributes.StatusAttribute.FLAG.SPOT
    },
    12: {
        value: entities.state.attributes.StatusAttribute.VALUE.ERROR
    },
    13: {
        value: entities.state.attributes.StatusAttribute.VALUE.IDLE
    },
    ...
    16: {
        value: entities.state.attributes.StatusAttribute.VALUE.MOVING,
        flag: entities.state.attributes.StatusAttribute.FLAG.TARGET
    },
    17: {
        value: entities.state.attributes.StatusAttribute.VALUE.CLEANING,
        flag: entities.state.attributes.StatusAttribute.FLAG.ZONE
    },
    18: {
        value: entities.state.attributes.StatusAttribute.VALUE.CLEANING,
        flag: entities.state.attributes.StatusAttribute.FLAG.SECTION
    },
    100: {
        value: entities.state.attributes.StatusAttribute.VALUE.DOCKED
    },
