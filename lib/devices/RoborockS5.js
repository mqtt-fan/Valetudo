...
class RoborockS5 extends Roborock {
    ...
    detectFanSpeeds(msg_ver) {
        ...
        if (msg_ver >= 3) {
            delete fanSpeeds[entities.state.attributes.FanSpeedAttribute.VALUE.MIN];
            fanSpeeds[entities.state.attributes.FanSpeedAttribute.VALUE.LOW] = 101;
            fanSpeeds[entities.state.attributes.FanSpeedAttribute.VALUE.MEDIUM] = 102;
            fanSpeeds[entities.state.attributes.FanSpeedAttribute.VALUE.HIGH] = 103;
            fanSpeeds[entities.state.attributes.FanSpeedAttribute.VALUE.MAX] = 104;
        }
            ...
            const flippedYCoordinates = persistantData.map(data => {
                if (data[0] === PERSISTENT_DATA_TYPES.ZONE) {
                    ...
                    return [
                        data[0],
                        data[1],
                        RRMapParser.DIMENSION_MM - data[2],
                        data[3],
                        RRMapParser.DIMENSION_MM - data[4],
                        data[5],
                        RRMapParser.DIMENSION_MM - data[6],
                        data[7],
                        RRMapParser.DIMENSION_MM - data[8]
                    ];
                } else {
                    return [
                        data[0],
